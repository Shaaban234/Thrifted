// Chat content guard. Blocks messages that try to take the deal off-platform
// (phone numbers, links, emails, files, or bank/wallet payment details) or that
// contain profanity — in English and Roman Urdu — so conversations stay on
// NayaPurana where Buyer Protection applies.
//
// NOTE: keep this file in sync with server/lib/moderation.ts — the two packages
// build separately, so the logic is intentionally duplicated (pure, no imports).
...
// Off-platform payment signals: mobile wallets, banks, and bank/card detail terms.
...
// Profanity (English + common Roman Urdu), matched on word boundaries.
const PROFANITY = [
  // English
...
// Multi-word Roman Urdu abuse (allow optional spacing between parts).
...
export function checkMessage(raw: string): GuardResult {
  ...
  // Links / URLs — explicit scheme, www., or a bare domain with a common TLD
  // (optionally with a path), e.g. "wa.me/123", "bit.ly/x", "myshop.store".
  ...
  // Bank / card / wallet payment details (checked before phone so a wallet
  // number reports the clearer "payment" reason).
  if (PAYMENT_PATTERNS.some((re) => re.test(text))) return blocked("payment");

  // File references / attachment links by extension
  if (/\.(pdf|docx?|xlsx?|pptx?|zip|rar|7z|apk|exe|dmg|jpe?g|png|gif|webp|mp4|mov|mkv|mp3|wav|csv|txt)\b/i.test(text))
    return blocked("file");

  // Phone numbers — a run of digits/separators holding at least 9 digits.
  // Slashes aren't separators here, which keeps dates like 12/03/2025 from matching.
